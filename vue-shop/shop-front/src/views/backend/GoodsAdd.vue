...
const defaultImg = '/static/images/404.jpg'
...
const getOptionLabel = (type) => {
  let prefix = ''
  if (type.fid !== 0) prefix = '├─ '
  if (type.fid !== 0 && !typeList.value.some(t => t.id === type.fid && t.fid === 0)) prefix = '  ├─ '
  return prefix + type.type_name
}

const getTypeList = async () => {
  try {
    const res = await request.get('/type/typeList', { params: { start: 0, count: 1000 } })
    if (res.status === 1) typeList.value = res.data.typeList
...
const resetForm = () => {
  goodsForm.good_name = ''
  goodsForm.img_url = defaultImg
  goodsForm.price = 0.01
  goodsForm.unit = ''
  goodsForm.type_id = ''
  goodsForm.intro = ''
  // 清除校验状态
...
const submitForm = async () => {
  try {
    ...
    const res = await request.post('/goods/addGood', goodsForm)
    if (res.status === 1) {
      ElMessageBox.alert('添加成功').then(() => {
        resetForm() // 重置表单
